...
int32_t mz_stream_wzaes_open(void *stream, const char *path, int32_t mode, zip64_info *zi) {
    ...
    if (mode & MZ_OPEN_MODE_WRITE) {
		...
		if (zi->write(salt_value, salt_length, zi) != salt_length)
			return MZ_WRITE_ERROR;

        wzaes->total_out += salt_length;
        printf("salt total out: %d\n", wzaes->total_out);

        /* if (mz_stream_write(wzaes->stream.base, verify, MZ_AES_PW_VERIFY_SIZE) != MZ_AES_PW_VERIFY_SIZE) */
        /*     return MZ_WRITE_ERROR; */
		if (zi->write(verify, MZ_AES_PW_VERIFY_SIZE, zi) != MZ_AES_PW_VERIFY_SIZE)
			return MZ_WRITE_ERROR;
		printf("verify = ");
		for (int i = 0; i < MZ_AES_PW_VERIFY_SIZE; i++)
			printf("%x ", verify[i]);
		printf("\n");

        wzaes->total_out += MZ_AES_PW_VERIFY_SIZE;
        printf("password verify total out: %d\n", wzaes->total_out);
    /* } else if (mode & MZ_OPEN_MODE_READ) { */
...
int32_t mz_stream_wzaes_write(void *stream, const void *buf, int32_t size, zip64_info *zi) {
    mz_stream_wzaes *wzaes = (mz_stream_wzaes *)stream;
    ...
    do {
		...
		written = zi->write(wzaes->buffer, bytes_to_write, zi);
...
int32_t mz_stream_wzaes_close(void *stream, zip64_info *zi) {
    mz_stream_wzaes *wzaes = (mz_stream_wzaes *)stream;
    ...
    if (wzaes->mode & MZ_OPEN_MODE_WRITE) {
        ...
        /*     return MZ_WRITE_ERROR; */
		if (zi->write(computed_hash, MZ_AES_AUTHCODE_SIZE, zi) != MZ_AES_AUTHCODE_SIZE)
			return MZ_WRITE_ERROR;

		printf("auth code: ");
		for (int i = 0; i < MZ_AES_AUTHCODE_SIZE; i++)
    ...
    /*     /1* If entire entry was not read this will fail *1/ */
